        ...
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          <div className="bg-white rounded-[2rem] p-6 md:p-8 border shadow-sm space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="font-black text-gray-900 uppercase text-[10px] md:text-xs tracking-widest flex items-center gap-2">
                <Receipt className="h-4 w-4 text-primary" /> Viáticos Adjuntos
              </h3>
              <span className="text-[10px] md:text-xs font-black text-gray-900 bg-gray-50 px-2 py-1 rounded-lg border">${expenses.reduce((acc, curr) => acc + curr.amount, 0).toLocaleString()} MXN</span>
            </div>

            <div className="space-y-3 md:space-y-4">
              {expenses.length > 0 ? expenses.map((exp) => (
                <div key={exp.id} className="p-4 md:p-5 border rounded-2xl md:rounded-3xl bg-gray-50/50 hover:bg-white transition-all group">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex gap-3 md:gap-4">
                      <div className="h-10 w-10 md:h-12 md:w-12 bg-white rounded-xl md:rounded-2xl border shadow-sm flex items-center justify-center text-primary shrink-0 relative overflow-hidden">
                        <div className="absolute inset-0 bg-gray-200 opacity-20" />
                        <FileText className="h-5 w-5 md:h-6 md:w-6 relative z-10" />
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs md:text-sm font-black text-gray-900 truncate">{exp.description}</p>
                        <p className="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest truncate">{exp.category} • {exp.date}</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-xs md:text-sm font-black text-gray-900">${exp.amount.toLocaleString()}</p>
                      <button className="text-[9px] md:text-[10px] font-black text-primary hover:underline uppercase tracking-tighter">
                        Ver Ticket
                      </button>
                    </div>
                  </div>
                ...
                <p className="text-center py-10 text-gray-400 font-bold italic text-sm">No hay gastos registrados.</p>
              )}
            </div>
          </div>

          {/* Reporte Técnico */}
          <div className="bg-white rounded-[2rem] p-6 md:p-8 border shadow-sm space-y-4">
            <h3 className="font-black text-gray-900 uppercase text-[10px] md:text-xs tracking-widest flex items-center gap-2">
              <FileText className="h-4 w-4 text-primary" /> Reporte de Ejecución
            </h3>
            <p className="text-gray-600 text-sm leading-relaxed font-medium bg-gray-50 p-5 md:p-6 rounded-2xl border border-dashed italic">
              "{ot.workDescription || 'Sin descripción adicional del reporte técnico.'}"
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6 md:space-y-8">
          <div className="bg-white rounded-[2rem] p-6 md:p-8 border shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="aspect-square bg-gray-100 rounded-xl md:rounded-2xl relative overflow-hidden group cursor-pointer border-2 border-transparent hover:border-primary transition-all shadow-sm">
                  <img 
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <Eye className="text-white h-5 w-5 md:h-6 md:w-6" />
                  </div>
                </div>
              ))}
            </div>
            
            <button className="w-full py-3 bg-gray-50 border-2 border-dashed rounded-xl md:rounded-2xl text-[10px] font-black text-gray-400 uppercase tracking-widest hover:border-primary/30 hover:text-primary transition-all">
          ...
          <div className="bg-white rounded-[2rem] p-6 md:p-8 border shadow-sm space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">Ubicación de Cierre</p>
                <p className="text-[10px] md:text-xs font-bold text-gray-900 truncate">Confirmado vía GPS por cuadrilla</p>
              </div>
            </div>
          </div>
        </div>
