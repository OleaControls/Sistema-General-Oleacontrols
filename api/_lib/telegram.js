...
export async function notifyOTAssigned(ot, techs) {
  ...
  const text = [
    ...
    ot.clientGoal ? `🎯 <b>Meta del cliente:</b> ${ot.clientGoal}` : null,
    ot.assignedFunds ? `💰 <b>Presupuesto inicial por viáticos:</b> $${ot.assignedFunds}` : null,
    ot.timeLimitHours ? `⏳ <b>Tiempo para completar:</b> ${ot.timeLimitHours} ${ot.timeLimitHours === 1 ? 'hora' : 'horas'}` : null,
    (ot.qualityHigh || ot.qualityMin) ? `` : null,
    (ot.qualityHigh || ot.qualityMin) ? `⭐ <b>Nivel de calidad esperado</b>` : null,
    ot.qualityHigh ? `   • <b>Alto:</b> ${ot.qualityHigh}` : null,
    ot.qualityMin  ? `   • <b>Mínimo:</b> ${ot.qualityMin}` : null,
    ``,
